...
uses uDemora, udmSaveFile, uAFIP;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    Epson.EspecificarNombreDeFantasia('�'+FantasyEpson1, '�'+FantasyEpson2);
                  end;
                end;
              end;
            except
...
begin
  ...
  if Mostrar then
  begin
    ShowMessage(' Punto de Venta = ' + PuntoCF + #13 + ' N� Fiscales =    ' + NoFiscales + #13 +
  ...
  case IF_Modelo of
    ...
    3:begin
      ...
      7:begin  //715 / PR5
          ...
          begin
            try
              ShowDemora('Abriendo Nota de Cr�dito ');
            ...
            begin
              ShowDemora('Abriendo Nota de D�bitos ');
    ...
    8:begin
        ...
        try
          ...
          begin
            ShowDemora('Abriendo Nota de Cr�dito [8]');
            CodNotaDeCredito := DocumentoValido(IF_Modelo, Factura.TipoFactura);
            if CodNotaDeCredito > NoEncontrado then
            begin
            ...
            if Factura.TipoFactura in [DebA, DebB] then
            begin
              ShowDemora('Abriendo Nota de D�bitos [8]');
              Hasar.DatosCliente(IntToStr(Factura.Entidad)+' '+Factura.NombreEnt, Factura.NroDocumento, TDocH, TIvaH, Factura.Direccion);
              Hasar.AbrirComprobanteFiscal(DocumentoValido(IF_Modelo, Factura.TipoFactura));
            end
            else begin // comprobante de Ventas
    ...
    4:begin // NCR 3140
        ...
        begin
          ...
          try
            ...
            begin
              if Factura.NombreEnt > Cero then
                SetDatos(10, '�'+'Cliente: '+Factura.NombreEnt)
              else
                SetDatos(10, ' ');
            end
            else begin
  ...
  begin
    ...
    try
      ...
      case Modelo_CF of
        ...
        3:begin  // 3= HASAR u OLIVETI PR4F
            ...
            begin // --------Combustibles ------------------------------
              ...
              begin
                ...
                begin
                  try
                    Hasar.ImprimirItem(Marca + ' Promoci�n', Cantidad, PrecioCero, AlicuotaIva, KIva)
                  except
                    Hasar.ImprimirItem(Marca + 'Promoci�n', Redondeo(PrecioCero, 2), Redondeo(Cantidad, 2), AlicuotaIva, KIva)
                  end;
                end
                else
                *)
        ...
        8:begin // HASAR u OLIVETTI PL8F � M320 � PJ20F
            Hasar.Modelo := Modelo_CF;
            try
              kIva := 1/(1 + (NoComputable/PrecioNetoT));
            except
            ...
            begin
              ...
              begin
                ...
                if InverCant then
                  ...
                  begin
                    try
                      Hasar.ImprimirItem(DetalleProducto + ' Promoci�n', Producto.Cantidad, PrecioCero, Producto.AlicuotaIva, KIva)
                    except
                      Hasar.ImprimirItem(DetalleProducto + ' Promoci�n', PrecioCero, Producto.Cantidad, Producto.AlicuotaIva, KIva)
                    end;
                  end
                  else
                  *)
              ...
              begin
                ...
                if Bonificacion > 0 then
                  ...
                  begin
                    try
                      Hasar.ImprimirItem(DetalleProducto + ' Promoci�n', Producto.Cantidad, PrcItmCero, Producto.AlicuotaIva, KIva)
                    except
                      Hasar.ImprimirItem(DetalleProducto + ' Promoci�n', PrcItmCero, Producto.Cantidad, Producto.AlicuotaIva, KIva)
                    end;
                  end
                  else begin
                  end;
...
begin
  ...
  // <--- [MODIFICADO] Agregamos "or (IF_Modelo = 99)" para permitir guardar Factura Electrónica
  if ControladorOK or (IF_Modelo = 99) then
  begin
    try
      case Modelo_CF of
        
        // <--- [NUEVO] Caso Modelo 99: Factura Electrónica
        99: begin 
             // No hacemos nada físico. El CAE ya se obtuvo en Imprimir_Comprobante.
             // Solo dejamos pasar el flujo para que se guarde en la BD abajo.
             ComprobanteOk := True; 
        end;
            ...
            try
              ...
              begin
                ...
                begin
                  for i := 1 to PagosTarjeta.CantTarjetas do
                    with PagosTarjeta.Movimientos[i] do
                      Hasar.ImprimirPago(NomTarjeta+' P '+IntToStr(CantidadCuotas)+' C '+Cupon+'/'+Lote, TotalTarj);
                end;
                ...
                if Valores in PagosRealizados then
                  Hasar.ImprimirPago('Cheques 3: ', PagosValores.TotalValores);
                ...
                if NotaCre in PagosRealizados then
                  Hasar.ImprimirPago('NC '+PagosNCred.NroNotaCre, PagosNCred.MtoNotaCre);
              ...
              if Factura.Empleado > 0 then
                ...
                SetDatos(13, '');
                SetDatos(14, 'DEF. CONS. SAN JUAN TEL:430 6400/05');
        ...
        6:begin
            try
              ...
              begin
                ...
                if Tarjeta in PagosRealizados then
                  for i := 1 to PagosTarjeta.CantTarjetas do
                    Epson.ImprimirPago('Tarjeta N '+ PagosTarjeta.Movimientos[i].Cupon, PagosTarjeta.Movimientos[i].TotalTarj);
                if Bonos in PagosRealizados then
              ...
              if Length(Factura.NombreEmpleado) > 0 then
                SetDatos(11,'Vendedor: ' + Factura.NombreEmpleado)
              //  SetDatos(11,'DEF.CONS.T:430 6400/05 V:' + Factura.NombreEmpleado)
              else
                SetDatos(11, '');
              if Length(Trim(Cartel)) > 0 then
                SetDatos(13,Cartel)
      ...
      begin
        ...
        begin
          try
            ...
            // <--- [MODIFICADO] Solo actualizamos el PtoVta si NO es Modelo 99
            if Modelo_CF <> 99 then 
...
begin
  case IF_Modelo of
    ...
    3:begin
        ...
        Hasar.ImprimirTextoNoFiscal('Entrega de Mercader�as');
        Hasar.ImprimirTextoNoFiscal('N� FACTURA ORIGINAL: ' + Factura.NroFactura);
        ...
        Hasar.ImprimirTextoNoFiscal('========================================');
        Hasar.ImprimirTextoNoFiscal('                                        ');
        Hasar.ImprimirTextoNoFiscal('Recib�: ________________________________');
        Hasar.CerrarComprobanteNoFiscal;
        ComprobanteOK := True;
      end;
    4,
    5:begin
        ...
        Epson.ImprimirTextoNoFiscal('Entrega de Mercader�as');
        Epson.ImprimirTextoNoFiscal('N� COMPROBANTE ORIGINAL: ' + Factura.NroFactura);
        ...
        Epson.ImprimirTextoNoFiscal('========================================');
        Epson.ImprimirTextoNoFiscal('                                        ');
        Epson.ImprimirTextoNoFiscal('Recib�: ________________________________');
      ...
      9:begin
          ...
          try
            ...
            if (Senas in DatosPago.PagosRealizados) then
              Linea := Linea + Format(' Se�as %m ', [DatosPago.PagosSena]);
            ...
            if (Senas in DatosPago.PagosRealizados) then
            begin
              Hasar.DetalleRecibo(Format('Se�as recibidas anteriormente: %m', [DatosPago.SdoAFavor]));
              Hasar.DetalleRecibo(Format('Sdo.a Favor operaci�n anterior: %m', [DatosPago.SdoAFavor]));
            end;
            *)
...
begin
  ...
  Case IF_Modelo of
    ...
    9:begin
        try
          ShowDemora('Abriendo cotizaci�n.');
          ...
          try
            ...
            raise EUsuario.Create('Error al abrir Cotizaci�n.');
          end;
          for Actual := 1 to Factura.CantArtic do
          begin
          ...
          Hasar.ImprimirItemEnCotizacion(Format('%-85s, %11.2m', ['           La presente cotizaci�n asciende a ', Factura.Total]));
  ...
  case IF_Modelo of
    ...
    begin
      ...
      begin
        ...
        try
          Hasar.AbrirComprobanteNoFiscal;
          Hasar.ImprimirTextoNoFiscal(#244 + 'Cheque(s) de 3� Ingresados');
          ...
          begin
            ...
            Hasar.ImprimirTextoNoFiscal(Format('Cheque N� %d - Imp: %m', [BancoTerNroCheque.Value, BancoTerMonto.Value]));
    ...
    begin
      ...
      try
        ...
        begin
          ...
          Epson.ImprimirTextoNoFiscal(Format('Cheque N� %d - Imp: %m', [BancoTerNroCheque.Value, BancoTerMonto.Value]));
          BancoTer.Next;
        end;
      finally
        BancoTer.Filter := '';
    ...
    begin
      ...
      begin
        if Tarjetas.Locate('IdTarjeta', DatosPago.PagosTarjeta.Movimientos[i].IdTarMut, []) then
          ...
          Hasar.ImprimirTextoNoFiscal(Format('Tarjeta: %s', ['Sin identificaci�n']));
        Hasar.ImprimirTextoNoFiscal(Format('N� Tarjeta: %s', [DatosPago.PagosTarjeta.Movimientos[i].NomTarjeta]));
    ...
    begin
      ...
      begin
        if Tarjetas.Locate('IdTarjeta', DatosPago.PagosTarjeta.Movimientos[i].IdTarMut, []) then
          ...
          Epson.ImprimirTextoNoFiscal(Format('Tarjeta: %s', ['Sin identificaci�n']));
        Epson.ImprimirTextoNoFiscal(Format('N� Tarjeta: %s', [DatosPago.PagosTarjeta.Movimientos[i].NomTarjeta]));
...
begin
  ...
  begin
    Application.MessageBox('Falta papel en el controlador/impresora fiscal' + #13 + 'Col�quelo, de Aceptar y espere' , 'Falta papel', MB_ICONEXCLAMATION);
    HuboFaltaPapel := True;
  end
  else begin
    Esta := False;
...
begin
  // 1. Configuración original de Epson (NO TOCAR)
  Epson := TEpson.Create;
  Epson.OnFaltaPapel := EpsonFaltaPapel;
  Epson.OnImpresoraOK := EpsonImpresoraOK;
  HuboFaltaPapel := False;

  // 2. NUEVA Configuración de AFIP (AGREGAR ESTO)
  // Aquí creamos el objeto para poder usarlo más tarde
  oAFIP := TAFIPConnector.Create(
      '20123456789',                                      // CUIT (Cámbialo por el real)
      'C:\Sistema\Certificados\certificado.crt',          // Ruta Certificado
      'C:\Sistema\Certificados\clave.key',                // Ruta Llave
      'https://wsaa.afip.gov.ar/ws/services/LoginCms',    // URL Login
      'https://servicios1.afip.gov.ar/wsfev1/service.asmx' // URL Facturación
  );
end;
...
var
  i: Integer;
  RespCAE: TRespuestaCAE; // <--- [NUEVO] Variable local para recibir respuesta de AFIP
begin
  try
    // <--- [NUEVO] INICIO BLOQUE FACTURA ELECTRÓNICA
    if IF_Modelo = 99 then
    begin
       ShowDemora('Conectando con AFIP...');
       try
         // Llamamos a la función principal de AFIP enviando cabecera e items juntos
         RespCAE := oAFIP.ObtenerCAE(Factura.TipoFactura, Factura.PuntoVta, Factura, ItemsFactura);

         if RespCAE.Resultado = 'A' then
         begin
            // ÉXITO: Guardamos CAE y Vencimiento en la estructura Factura (definida en uTools)
            Factura.CAE := RespCAE.CAE;
            Factura.VencimientoCAE := RespCAE.Vencimiento;
            
            // Si AFIP devuelve el número de comprobante asignado, lo usamos
            // Factura.NroFactura := ... (Opcional, depende de tu implementación de uAFIP)

            ComprobanteOk := True;
            
            // Llamamos a Cerrar para que guarde en BD, pero ya sin emitir ticket físico
            Cerrar_Comprobante(DatosPago, Factura, Text_PieComprobante);
         ...
         begin
            // FALLO: Rechazamos la operación y mostramos error
            Result := False;
            ComprobanteOk := False;
            raise Exception.Create('AFIP Rechazó: ' + RespCAE.Observaciones);
         end;
       finally
         CloseDemora;
       end;
       Exit; // <--- [IMPORTANTE] Salimos aquí para NO ejecutar código de Hasar/Epson
    end;
    // <--- [FIN BLOQUE FACTURA ELECTRÓNICA]

    // --- LÓGICA ORIGINAL (Se mantiene para modelos != 99) ---
    Result := True;
    if not ControladorOK then
      InicializarControlador;
    ImprimirEncabezado(Factura);
