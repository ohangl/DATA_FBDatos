...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage('No se pudo realizar la actualización. Terminal sin CF.-');
  end;
end;
...
begin
  ...
  begin // Emisión de comprobantes Con IF
  ...
  else begin // Emisión de comprobantes Con Impresora Comun
...
begin
  if (Application.MessageBox('¿Imprimir reporte X?', 'Confirmación',
...
begin
  if (Application.MessageBox('¡Última operación de la jornada fiscal!',
      'Atención', MB_ICONEXCLAMATION + MB_YESNO + MB_DEFBUTTON2) = ID_YES) then
  begin
    with frmFiscal do
    begin
      if not ControladorOK then
...
begin
  if not Check_Seg(Nivel_Adm, 'Counters', AutorDto) then
    raise EUsuario.Create('No esta habilitado.');
  if (Application.MessageBox('¿Acepta corregir costos en facturación de venta?',
  ...
  begin
    try
      ...
      ShowMessage('finalizado reparación de costos!!');
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      lstFormulario.PrinterSetup.DocumentName := tFormularioTipoComp.AsString +' Nº '+ tFormularioNroComp.AsString;
...
begin
  ...
  begin
    if not(Validate_Documento_Fiscal(Sender.Value))and
      (Application.MessageBox('El número de C.U.I.T./C.U.I.L. puede ser erroneo. Acepta?',
...
begin
  gConectados.ColumnByName('UEntrada').FooterValue := Format('%d',[C_In]);
  gConectados.ColumnByName('USalida').FooterValue := Format('%d',[C_Ot]);
  gConectados.ColumnByName('Nombre').FooterValue := Format('º Terminales %d',[C_Tr]);
end;
