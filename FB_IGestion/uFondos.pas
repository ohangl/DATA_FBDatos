...
Const
  ...
  ItmDet = 'Select A.IdFactura, A.IdSucursal, A.IdPuntoVenta, A.DiaHora, '+
           '       A.IdArticulo, A.FechaF, A.Cantidad, A.Descuento, '+
           ...
           '       A.RemSucSal, A.RemFacSal,K.tarjeta Nombre, A.IdArea, A.IdGrupo, '+
           '       A.IdSubGrupo, A.DetSuc,K.tarjeta '+
           'From PRODUCTOS_VENDIDOS_CAJA(:Desde, :Hasta, :IdSuc) A '+
           'Join FacVen F on F.IdFactura=A.IdFactura and F.IdSucursal=A.IdSucursal '+
            'Left  Join Tarjetas K '+
...
begin
  ...
  begin
    ...
    begin
      TotMonedaV.Append;
      TotMonedaVDetalle.AsString := 'Señas';
      ...
      TotalesMoneda.Append;
      TotalesMonedaDetalle.AsString := 'Señas';
  ...
  begin
    ...
    begin
      TotMonedaC.Append;
      TotMonedaCDetalle.AsString := 'Señas';
      TotMonedaCImporte.AsCurrency := qMoneyCSenas.AsCurrency;
      TotMonedaCPrcMda.AsFloat := (TotMonedaCImporte.AsCurrency*100) / TotalCob;
      TotMoneyC := TotMoneyC + qMoneyCSENAS.AsCurrency;
      TotMonedaC.Post;

      if TotalesMoneda.Locate(TotalesMonedaDetalle.FieldName, 'Señas', []) then
      begin
        TotalesMoneda.Edit;
...
begin
  ...
  try
    ...
    Screen.Cursor := crDefault;
    qSenas.Close;
    raise EUsuario.Create('Señas: no se puede calcular el periodo.- ');
  end;
  ...
  try
    ...
    Screen.Cursor := crDefault;
    qTotSen.Close;
    raise EUsuario.Create('Totales Señas: no se puede calcular el periodo.- ');
  end;
  ...
  try
    ...
    begin
      Screen.Cursor := crDefault;
      qSen_Ing.Close;
      raise EUsuario.Create('Señas Ingresadas: no se puede calcular el periodo.- '+E.Message);
    end;
  end;
  ...
  try
    ...
    Screen.Cursor := crDefault;
    qTotSenIng.Close;
    raise EUsuario.Create('Total Señas Ingresadas: no se puede calcular el periodo.- ');
  end;
...
begin
  qMovTarDetOper.AsString := Format('%s Nº %s %m Aº %s ',
