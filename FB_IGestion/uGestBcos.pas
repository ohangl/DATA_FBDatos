...
begin
  ...
  begin
    ...
    lblTituloConc.Caption := Format('Conciliación Bancaria Nº %d al %s del Banco %s ',
...
begin
  ...
  begin
    ...
    if c > 0 then
    begin
      if (Application.MessageBox('Ya existe un depósito con las mismas caracteristicas del que va ha realizar ¿Continua? ',
                                 'Depósito de Valores en Cartera', MB_ICONQUESTION+MB_YESNO) = ID_No) then
       Exit;
    end;
...
begin
  try
    ...
    begin
      ...
      ShowMessage('Ok. Depósito Bancos.- ');
...
begin
  if not Check_Seg(Nivel_Adm, 'Cheques 3º', AutorDto) then
...
begin
  ...
  begin
    if Trim(qBancoTerNROCOMPOP.AsString) > Vacio then
      lblSituacionChTer.Caption := 'Entregado OPº '+qBancoTerNROCOMPOP.AsString
    else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ChqModel.Post;
        if FileExists(ExtractFilePath(Application.ExeName)+'\cheque.rtm') then
        begin
          lstModelChq.Template.FileName := ExtractFilePath(Application.ExeName)+'\cheque.rtm';
          lstModelChq.Template.LoadFromFile;
        end;
